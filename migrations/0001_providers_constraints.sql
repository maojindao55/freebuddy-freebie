...
-- 为已存在的 D1 `providers` 表补上 schema.sql 中的 CHECK 约束（保留数据，不动其它表）。
--
-- 背景：schema.sql 使用 `CREATE TABLE IF NOT EXISTS`，只对全新数据库生效。已有库里的旧
-- `providers` 表结构宽松，重跑 schema.sql 不会让它获得约束，本迁移负责在保留数据的前提下重建它：
--
--   1. 先体检：把旧记录逐条判定（providers_migration_stage），并记录不合规的字段；
--   2. 新建带约束的 `providers_new`（结构与 schema.sql 的 providers 一致）；
--   3. 原样复制合规记录（pending / approved / rejected 全部保留，字段值不变）；
...
--     -- 推荐：由 wrangler 记录到 d1_migrations，失败的迁移会回滚。
...
--
-- 检查隔离出来的旧记录（确认后修正字段、重新投稿，或用 --json 导出留档）：
...
-- 注意：如果历史 `providers` 表缺少下面任何一列（结构被手改过），后面的 SELECT 会直接报
...
SELECT
  ...
  (
   ...
   AND status IS NOT NULL AND status IN ('pending', 'approved', 'rejected')
  ...
  trim(
    ...
    CASE WHEN (status IS NOT NULL AND status IN ('pending', 'approved', 'rejected')) IS NOT 1 THEN 'status ' ELSE '' END ||
...
-- 复制合规记录（pending / approved / rejected 全部保留，字段值不变）
