 ...
 *   generated SQL with `wrangler d1 execute --remote --command=` (the workflow reads the
 *   file and passes it as one argument, so D1's /import upload path is not involved; the
 *   equals form is required because the SQL starts with a `--` comment, which the wrangler
 *   CLI would otherwise parse as another option).
...
-- Applied by .github/workflows/sync-providers.yml (schema.sql first, then this file):
--   npx wrangler d1 execute freebie-db --remote --command="$(< <this file>)"
--   (equals form: this file starts with a -- comment, and wrangler parses a value that
--    starts with - as another option, so the space-separated form reports Unknown argument)
--
